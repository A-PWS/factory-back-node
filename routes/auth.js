...
router.post("/register", async (req, res) => {
  ...
  const reqRegEmail = [req.body.email];
  ...
  pool.query(
    ...
    async function (err, result, fields) {
      if (err) {
        ...
        if (result.length > 0) {
          return res.json({ code: 2, message: "Email is Taken" });
        } else {
          //hashing password
          ...
          const createdDate = convertToMySQLTimestamp(
